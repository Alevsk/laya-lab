...
export function targetsFor(action: ActionName, p: FlightParams = DEFAULT_FLIGHT): FlightTargets {
  switch (action) {
    case 'bank_left':
      return { vx: -p.lateralSpeed, vy: 0, speedMultiplier: 1 };
    case 'bank_right':
      return { vx: p.lateralSpeed, vy: 0, speedMultiplier: 1 };
    case 'climb':
      return { vx: 0, vy: p.verticalSpeed, speedMultiplier: 1 };
    case 'descend':
      return { vx: 0, vy: -p.verticalSpeed, speedMultiplier: 1 };
    case 'brake':
      return { vx: 0, vy: 0, speedMultiplier: p.brakeMultiplier };
    case 'forward':
      return { vx: 0, vy: 0, speedMultiplier: 1 };
  }
}
...
export function relaxVelocity(
  ...
  const vzTarget = -p.cruiseSpeed * targets.speedMultiplier;
